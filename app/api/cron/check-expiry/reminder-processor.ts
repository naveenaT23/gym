...
export async function processReminders() {
  ...
  const templates = settings.whatsapp_templates || {
    "7_days_before": "Hello {{customer_name}}, your {{plan_duration}} plan (joined on {{joining_date}}) will expire in 7 days on {{expiry_date}}. Kindly renew your membership to continue your fitness journey. Contact us for renewal. Thank you!",
    "3_days_before": "Hello {{customer_name}}, your {{plan_duration}} plan (joined on {{joining_date}}) will expire in 3 days on {{expiry_date}}. Kindly renew your membership to continue your fitness journey. Contact us for renewal. Thank you!",
    "on_expiry": "Hello {{customer_name}}, your {{plan_duration}} plan (joined on {{joining_date}}) expires today on {{expiry_date}}. Kindly renew your membership to continue your fitness journey. Contact us for renewal. Thank you!",
    "after_expiry": "Hello {{customer_name}}, your {{plan_duration}} plan (joined on {{joining_date}}) expired on {{expiry_date}}. Kindly renew your membership to continue your fitness journey. Contact us for renewal. Thank you!",
  };
  ...
  for (const [type, targetDateStr] of Object.entries(dates)) {
    ...
    for (const member of targetMembers) {
      ...
      const planDuration = 
        member.package_type === "Monthly" ? "1 Month (Monthly)" :
        member.package_type === "Quarterly" ? "3 Months (Quarterly)" :
        member.package_type === "Half-Yearly" ? "6 Months (Half-Yearly)" :
        member.package_type === "Yearly" ? "1 Year (Yearly)" :
        member.package_type || "membership";

      const formattedJoinDate = new Date(member.join_date).toLocaleDateString("en-IN", {
        day: "2-digit",
        month: "2-digit",
        year: "numeric",
      });
      ...
      msg = msg
        ...
        .replace(/{{plan_duration}}/g, planDuration)
        .replace(/{{joining_date}}/g, formattedJoinDate);
